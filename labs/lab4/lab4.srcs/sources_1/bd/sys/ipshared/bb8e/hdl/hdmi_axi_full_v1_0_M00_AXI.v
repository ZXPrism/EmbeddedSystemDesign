...
`timescale 1 ns / 1 ps
	...
	always @(posedge M_AXI_ACLK) begin
        if (~M_AXI_ARESETN || M_AXI_RLAST) axi_rready <= 1'd0;
        else if (M_AXI_ARVALID && M_AXI_ARREADY) axi_rready <= 1'd1;
        ...
        if (~M_AXI_ARESETN) begin
            ...
            case (txn_state)
                ...
                TXN_STATE_ACTIVE: begin
                    if (M_AXI_ARREADY && M_AXI_ARVALID && txn_cnt < 6'd60) begin
                        txn_state <= TXN_STATE_WAIT;
					...
					else if(txn_cnt == 6'd60) begin // 60 个 burst 块发送完毕，等待下一行开始... // 检查！边界条件可能出错！
                        txn_state <= TXN_STATE_INITIAL;
        ...
        if (~M_AXI_ARESETN) begin
			...
			case(addra_state)
				...
				ADDRA_STATE_READING: begin
					if(M_AXI_RREADY == 1'b1 && addra_reg < 16'd959) begin
						addra_reg <= addra_reg + 16'd1;
					end
					else if(addra_reg == 16'd959) begin
						addra_reg <= 16'd0;
						addra_state <= ADDRA_STATE_INITIAL;
					end
